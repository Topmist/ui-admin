...
public class GetCSARInstanceRunningPlansAction extends ActionSupport {
	...
	public String execute() {
		...
		List<String[]> value = client.getLinksFromUri(ContainerClient.BASEURI
				...
				+ this.internalInstanceID + "/activePublicPlans", false);
		...
		if (size > 0) {
			...
			for (String buildCorrelation : correlationList) {
				...
				PublicPlan publicPlan = client.getRunningPublicPlan(this.csarID, this.internalInstanceID, buildCorrelation);
				this.activePlans.add(invocationDate + " "
						+ publicPlan.getPlanID());
			}
		}
